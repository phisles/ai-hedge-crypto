...
AVAILABLE_MODELS = [
    ...
    LLMModel(
        display_name="[openai] o4-mini TEST 06-2025",
...
def get_model(model_name: str, model_provider: ModelProvider) -> ChatOpenAI | ChatGroq | ChatOllama | None:
    if model_provider == ModelProvider.GROQ:
        ...
        if not api_key:
            print(f"API Key Error: Please make sure OPENAI_API_KEY is set in config.py.")
            raise ValueError("OpenAI API key not found. Please make sure OPENAI_API_KEY is set in config.py.")
