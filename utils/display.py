...
def print_trading_output(result: dict) -> None:
    ...
    for ticker, decision in decisions.items():
        ...
        if not table_data:
            print(f"{Fore.YELLOW}⚠️ No agent analysis available for {ticker}{Style.RESET_ALL}")
            continue
        ...
        if not action:
            print(f"{Fore.YELLOW}⚠️ No trading decision found for {ticker}{Style.RESET_ALL}")
            continue
        decision_data = [
